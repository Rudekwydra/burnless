...
def render_metrics(m: dict, *, show_cost: bool = True) -> str:
    ...
    lines = [
        ...
        "",
        "Counters:",
        f"  encoder_calls   {m.get('encoder_calls', 0)}   (Maestro brain only)",
        f"  decoder_calls   {m.get('decoder_calls', 0)}   (Maestro brain only)",
        f"  brain_calls     {m.get('brain_calls', 0)}   (Maestro brain only)",
