...
def create_stream(
    ...
    *,
    ...
    try:
        ...
        raise ImportError("pip install openai to use OpenRouter Brain adapter")
    ...
    for chunk in stream:
        ...
        if not choices:
            # best-effort: check for reasoning field at chunk level
            reasoning = getattr(chunk, "reasoning", None)
            if isinstance(reasoning, str) and reasoning:
                yield NormalizedEvent(kind="think_delta", text=reasoning)
            continue
